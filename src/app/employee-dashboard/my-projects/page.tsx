...
const StatusIcon = ({ status }: { status: ProjectStatus }) => {
    switch (status) {
        ...
        case 'not-started': return <XCircle className="h-5 w-5 text-red-500" />;
        default: return null;
    }
};
            ...
            <Card>
                <CardHeader className="flex flex-row items-center gap-4">
                    <Avatar className="h-16 w-16 border-4 border-primary">
                        <AvatarFallback className="bg-secondary text-secondary-foreground font-bold text-2xl">{getInitials(user.name)}</AvatarFallback>
                    </Avatar>
                    <div>
